...
export default async function MembersPage() {
  ...
  const members = await listMembers(true, group);
  return <div className="page-wrap"><header className="page-header"><p className="eyebrow">{group ? `${group} roster` : "Roster"}</p><h1>{group ? `${group} members` : "Team members"}</h1><p>Only active members appear on new practice check-in sheets.</p></header><div className="two-column">
